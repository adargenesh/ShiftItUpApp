...
namespace ShiftItUptApp.Services
{
    public class ShiftItUptWebAPIProxy
    {
        ...
        */
        ...
        public async Task<WorkerShiftRequest?> AddShiftRequest(WorkerShiftRequest request)
        {
            //Set URI to the specific function API
            string url = $"{this.baseUrl}AddShiftRequest";
            try
            {
                //Call the server API
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    WorkerShiftRequest? result = JsonSerializer.Deserialize<WorkerShiftRequest>(resContent, options);
                    return result;
                }
                else
                {
        ...
        {
            //Set URI to the specific function API
            string url = $"{this.baseUrl}ApproveWorker";
            try
            {
                //Call the server API
                string json = JsonSerializer.Serialize(user);
        ...
        {
            string url = $"{this.baseUrl}getStatuses";
            try
            {
                // Call the server API
                HttpResponseMessage response = await client.GetAsync(url);
